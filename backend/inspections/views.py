...
def client_application_submit(request):
    ...
    app.application_pdf.save(filename, uploaded_pdf, save=True)

    if inspection is not None and not inspection.application_pdf:
        inspection.application_pdf.save(filename, app.application_pdf.file, save=True)
        mirror_inspection(inspection)

    return JsonResponse({
        "ok": True,
        "inspection": serialize_inspection(request, inspection) if inspection else None,
    })
